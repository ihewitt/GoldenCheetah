    ...
    if (telemetry) {
        ...
        if (lastMessage.type != 0) {
           ...
           case CHANNEL_TYPE_QUBO_DIGITAL:
           {
               ...
               //TODO add configuration option to run qubo in passive level mode?
               ...
               // In manual level mode, power is calculated in RealtimeController
               if (!parent->modeLevel())
               {
                   // TODO Move this into realtime controller too?
                   //      remove duplication of power logic?
                   //      just set active level here instead?
